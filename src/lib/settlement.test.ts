...
describe("Debt Simplification Algorithm", () => {
  it("should split a simple $100 bill correctly", () => {
    ...
    const receipts = [
      { id: "r1", uploader_id: "alice", exchange_rate_to_base: 1, cc_fee_percentage: 0 }
    ];
    const items = [
      { id: "i1", receipt_id: "r1", item_name: "Dinner", price: 100, assigned_to: ["alice", "bob"] }
    ];

    const { calculatedSettlements, userBalances } = calculateSettlements(items, receipts, participants);
  ...
  it("should handle credit card fee percentages", () => {
    const participants = [{ id: "alice" }, { id: "bob" }];
    const receipts = [
      { id: "r1", uploader_id: "alice", exchange_rate_to_base: 1, cc_fee_percentage: 3 } // 3% fee
    ];
    const items = [
      { id: "i1", receipt_id: "r1", item_name: "Dinner", price: 100, assigned_to: ["alice", "bob"] }
    ];
  ...
  it("should handle cyclic debts automatically", () => {
    ...
    const items = [
      { id: "i1", receipt_id: "r1", item_name: "Lunch", price: 30, assigned_to: ["B"] },
      { id: "i2", receipt_id: "r2", item_name: "Dinner", price: 30, assigned_to: ["C"] },
      { id: "i3", receipt_id: "r3", item_name: "Breakfast", price: 30, assigned_to: ["A"] }
    ];

    const { calculatedSettlements } = calculateSettlements(items, receipts, participants);
