...
import { 
  Copy, Check, Camera, Smile, Cat, Dog, Zap, 
  Star, Heart, Rocket, User, Users, Loader2 
} from "lucide-react"

// --- Config Data ---
const AVATARS = {
  smile: Smile, cat: Cat, dog: Dog, zap: Zap,
  star: Star, heart: Heart, rocket: Rocket, user: User
}
...
export default function RoomPage({ params }: { params: { roomCode: string } }) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      channel = supabase.channel(`room_${sessionData.id}`)
        .on(
          'postgres_changes', 
          { event: '*', schema: 'public', table: 'participants', filter: `session_id=eq.${sessionData.id}` }, 
      ...
      <main className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
        <div className="max-w-md w-full bg-white p-6 sm:p-8 rounded-3xl shadow-xl border border-gray-100">
          <div className="text-center mb-8">
            <h1 className="text-2xl font-bold text-gray-900">Join Room <span className="text-indigo-600 font-mono tracking-wider">{roomCode}</span></h1>
            <p className="text-gray-500 mt-1">Pick your gametag and avatar!</p>
          </div>

          {error && <div className="text-red-600 text-sm text-center mb-4 bg-red-50 p-2 rounded-lg">{error}</div>}
          ...
          <form onSubmit={handleJoin} className="space-y-6">
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 transition-all text-lg font-medium outline-none"
              />
            </div>

            <div className="space-y-3">
              <label className="text-sm font-semibold text-gray-700">Pick an Avatar</label>
                ...
                {(Object.keys(AVATARS) as AvatarKey[]).map((iconKey) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "aspect-square rounded-2xl flex items-center justify-center transition-all bg-gray-50 border-2",
                        isSelected ? "border-indigo-600 bg-indigo-50 text-indigo-600" : "border-gray-100 text-gray-400 hover:border-gray-200 hover:text-gray-600"
                      )}
                    >
                      <IconComponent className={cn("w-7 h-7", isSelected && "scale-110")} />
                    </button>
                  )
                })}
              </div>
            ...
            <div className="space-y-3">
              <label className="text-sm font-semibold text-gray-700">Pick a Color</label>
                ...
                {COLORS.map((c) => (
                  <button
                    ...
                    className={cn(
                      ...
                      c.class,
                      selectedColor === c.id ? "ring-4 ring-offset-2 ring-indigo-200 scale-110" : "opacity-80 hover:opacity-100 hover:scale-105"
                    )}
                  >
                    {selectedColor === c.id && <Check className="w-5 h-5 text-white stroke-[3px]" />}
                  </button>
                ))}
              </div>
            </div>
            ...
            >
              {isSubmitting ? <Loader2 className="w-6 h-6 animate-spin" /> : "Enter Lobby"}
            </button>
          </form>
        </div>
      </main>
  ...
  return (
    <main className="min-h-screen bg-gray-50 flex flex-col p-4 max-w-md mx-auto relative pt-12 pb-24">
      ...
      <div className="text-center space-y-4 mb-8">
        <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Lobby</h1>
        ...
        >
          <span className="font-mono text-xl font-bold tracking-widest text-indigo-700">{roomCode}</span>
          <div className={cn(
      ...
      <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm mb-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          {participants.map((p) => {
            ...
            const colorClass = COLORS.find(c => c.id === p.avatar_color)?.class || 'bg-gray-800'
            ...
            return (
              <div key={p.id} className="flex flex-col items-center gap-1.5 relative group">
                ...
                {isMe && <span className="absolute -top-2 -right-1 text-[10px] bg-indigo-600 text-white px-1.5 py-0.5 rounded-full font-bold">ME</span>}
              </div>
            )
          })}

          {/* Empty placeholders to suggest more people can join */}
          {participants.length < 8 && Array.from({ length: 8 - participants.length }).map((_, i) => (
             <div key={`empty-${i}`} className="flex flex-col items-center gap-1.5 opacity-40">
